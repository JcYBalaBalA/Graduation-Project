...
# 2.传入disPath、disMod，路径所代表的是通过torch.save(net.state_dict,PATH)保存的参数
# 而disMod代表具有某些参数的目标模型，通过disMod.load_state_dict载入这些参数
# 3.传入disMod，直接传入所要攻击的模型

# 传入的trainloader及testloader仅代表获取数据时默认的训练集和测试集,若只需要一个输入可将testloader置为None
# return 长度为trainsize的样本数组
def attack(trainloader, testloader, trainsize, cuda, disPath = None, disMod = None):
    if disPath != None and disMod == None:
        disMod = torch.load(disPath)
    elif disPath != None and disMod != None:
        disMod = disMod.load_state_dict(torch.load(disPath))
...
# 实验用
# return 白盒攻击预测准确度
def attack_show(trainloader, testloader, trainsize, cuda, disPath = None, disMod = None):
    if disPath != None and disMod == None:
        disMod = torch.load(disPath)
    elif disPath != None and disMod != None:
        disMod = disMod.load_state_dict(torch.load(disPath))
